...
func (s *SQLAssignmentStore) GetByID(id int) (*models.Assignment, error) {
	...
	if err != nil {
		...
		log.Printf("Error fetching assignment by ID %d: %v", id, err)
		return nil, err
	}
...
func (s *SQLAssignmentStore) Delete(id int) error {
	...
	if err != nil {
		log.Printf("Error deleting assignment with ID %d: %v", id, err)
		return err
	}
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		log.Printf("Error getting rows affected for assignment ID %d: %v", id, err)
		return err
	}
	if rowsAffected == 0 {
		return ErrNotFound
...
func (s *SQLAssignmentStore) GetAssignmentHistoryForChild(childID int) ([]models.Assignment, error) {
	...
	if err != nil {
		log.Printf("Error fetching assignment history for child ID %d: %v", childID, err)
		return nil, err
	}
	defer rows.Close()
	...
	if err = rows.Err(); err != nil {
		log.Printf("Error iterating over assignment history for child ID %d: %v", childID, err)
		return nil, err
	}
...
func (s *SQLAssignmentStore) EndAssignment(assignmentID int) error {
	...
	if err != nil {
		log.Printf("Error ending assignment with ID %d: %v", assignmentID, err)
		return err
	}
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		log.Printf("Error getting rows affected for ending assignment ID %d: %v", assignmentID, err)
		return err
	}
	if rowsAffected == 0 {
		return ErrNotFound
